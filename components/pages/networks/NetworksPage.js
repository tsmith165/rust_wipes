...
const NetworksPage = () => {
    ...
    for (const network of state.networks) {
        ...
        for (const server of network.servers.sort((a, b) => a.rank - b.rank)) {
            ...
            network_server_list_jsx.push(
                <Link href={`/server/${server.id}`}>
                    <div
                        key={server.id}
                        className={`text-light px-2.5 py-1 truncate hover:bg-primary hover:text-dark ${
        ...
        network_card_list_jsx.push(
            <div
                ...
                <div className="flex flex-col h-full bg-dark rounded-lg">
                    <h3 className="text-primary p-4">{network.name}</h3>
                    <div className="flex-1 bg-tertiary last:rounded-b-lg">
    ...
    return (
        <div className="flex flew-row h-full w-full overflow-x-auto bg-medium">
