...
const ServerInfoPanel = ({ bm_api_attributes, database_data }) => {
    ...
    const server_info_data = [
        ...
        { title: 'Server Region', value: (region in region_dict) ? region_dict[region] : region },
    ...
    return (
        <div className="bg-medium hover:bg-light rounded-lg p-2.5">
            ...
            {server_info_data.map(({ title, value }) => (
                <p className="hover:font-bold hover:text-md hover:text-lg hover:text-primary" key={title}>{title}: {value}</p>
            ))}
        </div>
    );
};
