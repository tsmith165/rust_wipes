    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="custom-tooltip bg-white p-3 border rounded-md shadow-md">
                ...
                <p className="desc">
                    <span className="dot" style={{color: '#8884d8'}}>&#x25CF; </span>
                    Max: {payload[0].payload.max}
                </p>
                <p className="desc">
                    <span className="dot" style={{color: '#82ca9d'}}>&#x25CF; </span>
                    Value: {payload[0].payload.value}
                </p>
                <p className="desc">
                    <span className="dot" style={{color: '#ff7300'}}>&#x25CF; </span>
...
const PlayerCountPanel = ({ player_count_data }) => {
    ...
    const formatted_data = player_count_data?.data?.map((item) => ({
