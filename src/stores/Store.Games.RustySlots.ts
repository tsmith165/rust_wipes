...
export const useSlotGame = create<SlotGameState>((set, get) => {
    // Create stable functions outside the state object
    const setGrid = (grid: string[][]) => {
        const state = get();
        // Skip update if the grid structure is the same (length and content)
        if (
            state.currentGrid.length === grid.length &&
            state.currentGrid.every((row, i) => grid[i] && row.length === grid[i].length && row.every((cell, j) => cell === grid[i][j]))
        ) {
            return;
        }
    ...
    return {
        ...
        cyclePossibleLines: () =>
            set((state) => {
                ...
                if (newFlashCount >= 6) {
                    const groups: ('horizontal' | 'vShaped' | 'invertedV' | 'diagonal')[] = [
                        'horizontal',
                        'vShaped',
                        'invertedV',
                        'diagonal',
