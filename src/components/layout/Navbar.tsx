    ...
    const navbar = navbar_menu_list.map(([menu_class_name, menu_full_name, href]) => {
        ...
        return (
            <div
                ...
                className={`h-full cursor-pointer items-center justify-center font-bold ${['faq'].includes(menu_class_name) ? 'hidden' : ['recent'].includes(menu_class_name) ? 'hidden xxs:!flex' : ['upcoming'].includes(menu_class_name) ? 'hidden xs:!flex' : ['networks', 'stats', 'wheel'].includes(menu_class_name) ? 'hidden sm:!flex' : 'flex'} ${page.includes(menu_class_name) ? 'radial-gradient-stone-300 bg-primary bg-clip-text text-transparent' : 'radial-gradient-stone-300 bg-stone-600 bg-clip-text text-transparent hover:!bg-primary'}`}
            >
                {menu_full_name}
            </div>
        );
