// /lib/battlemetrics_scrapper/simple_bm_scrapper.js
...
const { ATTRIBUTE_GROUPS, ATTRIBUTE_KEYWORDS } = require('./bm_scraper_attributes');
const {
    BM_API_BASE_URL,
    BASE_FILTER,
    COUNTRY_FILTER,
    ...
    MY_DATE_FORMAT,
    BM_DATE_FORMAT,
} = require('./bm_scraper_constants');
const { fetch_server_list, count_keywords_in_string } = require('./scraper_generic_helper');
const { insert_scrapper_stats, output_stats } = require('./scraper_stats_helper');
...
class SimpleScrapper {
    ...
    async run() {
        ...
        this.runInterval(run_interval)
            .then(() => {
                console.log(
                    `Scrapper has been set up to run at ${run_interval / 1000} second intervals.`
                );
            })
            .catch((err) => {
    ...
    async runInterval(interval) {
        ...
        const intervalRun = async () => {
            try {
                await this.run_scraper(); // This is your existing method to run the scrapper
    ...
    async parse_single_server(server) {
        ...
        var { id, rank, name, players, maxPlayers, details, rust_description, rust_last_wipe } =
            attrs;
        rust_description = details ? details.rust_description : '';
        rust_last_wipe = details ? details.rust_last_wipe : null;

        const wipeDateTime = rust_last_wipe
            ? moment(new Date(rust_last_wipe)).format(BM_DATE_FORMAT)
            : null;
        ...
        if (rank > this.min_rank) {
            this.log(
                `Not saving servers with rank > ${this.min_rank} in DB. Skipping ${name}. Amount skipped:', this.servers_skipped`
            );
            this.servers_skipped++;
            return;
        }
        ...
        if (wipeDateTime && moment().diff(wipeDateTime, 'days') > this.max_days_old) {
            this.log(
                `Wipe is older than ${this.max_days_old} days old. Skipping. Amount skipped:', this.servers_skipped`
            );
            this.servers_skipped++;
            return;
        }
        ...
        if (!wipeDateTime) {
            this.servers_skipped++;
            this.log(
                'Skipping because wipe time already exists. Amount skipped:',
                this.servers_skipped
            );
            return;
        }
        ...
        const [wipe_array, most_frequent_wipe, second_frequent_wipe, new_record] =
            await this.search_existing(id, wipeDateTime, force_wipe);
    ...
    async insert_into_db(data) {
        ...
        secondary_day =
            typeof secondary_day === 'number' && !isNaN(secondary_day) ? secondary_day : null;
        secondary_hour =
            typeof secondary_hour === 'number' && !isNaN(secondary_hour) ? secondary_hour : null;
    ...
    async search_existing(bm_id, wipe_time, force_wipe = false) {
        ...
        this.log(
            'Wipe time does not exist in wipe array. Adding current wipe time to wipe array...'
        );
        wipe_array.unshift(wipe_time);

        const wipe_dict = this.create_wipe_timestamp_dict(wipe_array, false);
        const { most_frequent_wipe, second_frequent_wipe } =
            this.determine_frequent_wipes(wipe_dict);
    ...
    determine_frequent_wipes(wipe_dict) {
        ...
        return {
            ...
            second_frequent_wipe: [
                second_frequent_day,
                second_frequent_hour,
                second_frequent_timestamp,
            ],
        };
    }
    ...
    parse_grouped_attributes_for_max(serverAttributes) {
        ...
        for (const [groupKey, attributes] of Object.entries(serverAttributes)) {
            ...
            if (groupKey === 'group_limit') {
                for (const groupLimit of ['no limit', 'quad', 'trio', 'duo', 'solo']) {
                    if (attributes[groupLimit]) {
                        ...
                        this.server_attribute_stats[groupLimit] =
                            (this.server_attribute_stats[groupLimit] || 0) + 1;
                        break;
                    }
                }
                continue;
            ...
            maxAttributes[groupKey] = Object.keys(attributes).reduce((a, b) =>
                attributes[a] > attributes[b] ? a : b
            );
            this.server_attribute_stats[maxAttributes[groupKey]] =
                (this.server_attribute_stats[maxAttributes[groupKey]] || 0) + 1;
        }
    ...
    parse_server_attributes(title, description) {
        ...
        for (const [groupKey, attributeKeys] of Object.entries(ATTRIBUTE_GROUPS)) {
            ...
            for (const attribute of attributeKeys) {
                ...
                const count =
                    count_keywords_in_string(keywords, title) +
                    count_keywords_in_string(keywords, description);
                if (count > 0) {
                    this.server_attribute_stats[groupKey][attribute] =
                        (this.server_attribute_stats[groupKey][attribute] || 0) + count;
                }
            }
        }
    ...
    create_bm_server_list_api_call_string() {
        const api_call_string = `${BM_API_BASE_URL}?${BASE_FILTER}&${COUNTRY_FILTER}=${this.country}&${DISTANCE_FILTER}=${this.distance}&${PLAYERS_FILTER}=${this.min_players}&${PAGE_LEN_FILTER}=${this.page_length}`;
        this.log(`Server List API Call: ${api_call_string}`);
        return api_call_string;
    }
}

module.exports = SimpleScrapper;
