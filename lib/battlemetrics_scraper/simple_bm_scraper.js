...
class SimpleScrapper {
    ...
    async parse_single_server(server) {
        ...
        if (rank > this.min_rank) {
            this.log(`Not saving servers with rank > ${this.min_rank} in DB. Skipping ${name}...`);
            this.servers_skipped++;
            return;
        }
        // Server has no wipe time and is not force wipe
        if (wipeDateTime && moment().diff(wipeDateTime, 'days') > this.max_days_old) {
            this.log(`Wipe is older than ${this.max_days_old} days old. Skipping...`);
            this.servers_skipped++;
            return;
        }
        ...
        if (new_record === false && !wipe_array) {
            this.log('Skipping because wipe time already exists.');
            return;
        }
