...
const { BM_API_BASE_URL, BASE_FILTER, COUNTRY_FILTER, DISTANCE_FILTER, PLAYERS_FILTER, PAGE_LEN_FILTER, MY_DATE_FORMAT, BM_DATE_FORMAT} = require('./bm_scraper_constants');
...
class SimpleScrapper {
    ...
    async run() {
        ...
        output_stats(this.start_time, this.end_time, this.servers_parsed, this.servers_skipped, this.servers_posted, this.server_attribute_stats);
    ...
    async parse_single_server(server) {
        ...
        if (!wipeDateTime) { this.servers_skipped++; return; }
