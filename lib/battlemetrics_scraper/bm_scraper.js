...
const { BM_API_BASE_URL, BASE_FILTER, COUNTRY_FILTER, DISTANCE_FILTER, PLAYERS_FILTER, PAGE_LEN_FILTER, WIPE_FILTER, MY_DATE_FORMAT, BM_DATE_FORMAT, REGION_MAP } = require('./bm_scraper_constants');
...
class Scrapper {
    ...
    async run() {
        ...
        output_stats(this.start_time, this.end_time, this.servers_parsed, this.servers_skipped, this.servers_posted);
        await insert_scrapper_stats(this.prisma, this.start_time, this.end_time, this.servers_parsed, this.servers_skipped, this.servers_posted);
    ...
    async parse_server_list_data(response) {
        ...
        for (const server of response.data) {
            ...
            if (rank > this.min_rank) {
                console.log(`Not saving servers with rank > ${this.min_rank} in DB. Skipping ${name}...`);
                this.servers_skipped++;
                continue;
            }
            ...
            if (wipeDateTime && moment().diff(wipeDateTime, 'days') > this.max_days_old) {
                ...
                this.servers_skipped++; continue;
            }
            // Server has no wipe time
            if (!wipeDateTime) { this.servers_skipped++; continue; }
