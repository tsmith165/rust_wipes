...
const { output_stats, insert_scrapper_stats } = require('./scraper_stats_helper');
...
class Scrapper {
    constructor({ max_days_old, min_rank }) {
        this.max_days_old = max_days_old;
        this.min_rank = min_rank;
        this.page_length = 50;
        this.max_pages = 9999999999;
        this.distance = 5000;
        this.min_players = 2;
        this.country = 'US';
        this.today = moment();
    ...
    async parse_server_list_data(response) {
        ...
        for (const server of response.data) {
            ...
            console.log(
                `Last Wipe: ${wipeDateTime ? wipeDateTime : 'N/A'} | Force Wipe?: ${force_wipe}`
            );
            ...
            if (rank > this.min_rank) {
                console.log(
                    `Not saving servers with rank > ${this.min_rank} in DB. Skipping ${name}...`
                );
                this.servers_skipped++;
                continue;
            }
            ...
            if (wipeDateTime && moment().diff(wipeDateTime, 'days') > this.max_days_old) {
                console.log(`Wipe is older than ${this.max_days_old} days old. Skipping...`);
                this.servers_skipped++;
                continue;
            }
    ...
    create_bm_server_list_api_call_string() {
        var api_call_string = `${BM_API_BASE_URL}?${BASE_FILTER}&${COUNTRY_FILTER}=${this.country}&${DISTANCE_FILTER}=${this.distance}&${PLAYERS_FILTER}=${this.min_players}&${PAGE_LEN_FILTER}=${this.page_length}&${WIPE_FILTER}`;
