...
async function fetch_upcoming_servers_for_week_day(date, time_zone, min_rank, region, resource_rate, group_limit, game_mode, absolute=false) {
    const final_server_addr = (process.env.NODE_ENV === 'development') ? 'http://localhost:3000' : website_addr;
...
async function fetch_scraper_stats_data(current_page=1, items_per_page=5) {
    const final_server_addr = (process.env.NODE_ENV === 'development') ? 'http://localhost:3000' : website_addr;
    try {
        const response = await axios.post(`${final_server_addr}/api/scraper/stats`, {
...
async function fetch_recent_wipes_from_db() {
    const final_server_addr = (process.env.NODE_ENV === 'development') ? 'http://localhost:3000' : website_addr;
    const api_call_url = `${final_server_addr}/api/wipes/recent`
    ...
    try {
        ...
        console.error("Fetching most recent 25 servers from DB failed. (Traceback next line)");
        console.log(error);
