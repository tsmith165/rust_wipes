const server_addr = (process.env.NODE_ENV == 'development') ? 'http://localhost:3000' : 'https://rustwipes.net';

async function fetch_upcoming_servers_for_week_day(date, time_zone, min_rank) {
    console.log(`Fetching upcoming servers for date: ${date} | min rank: ${min_rank}`)
    try {
        const response = await fetch(`${server_addr}/api/upcoming/servers/`, {
            headers: {
