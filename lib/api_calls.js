...
async function fetch_upcoming_servers_for_week_day(
    ...
    const final_server_addr =
        process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : '';
    console.log(
        `Fetching upcoming servers for date: ${date} | min rank: ${min_rank}`,
    );
    try {
        const response = await axios.post(
            `${final_server_addr}/api/upcoming/`,
            {
                date: date,
                time_zone: time_zone,
                min_rank: min_rank,
        ...
        console.error(
            'Fetching upcoming servers api call failure.  (Traceback next line)',
        );
        console.log(error);
...
async function fetch_scraper_stats_data(current_page = 1, items_per_page = 5) {
    const final_server_addr =
        process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : '';
    try {
        const response = await axios.post(
            `${final_server_addr}/api/scraper/stats`,
            {
        ...
        console.log('Scraper Stats data (Next Line):');
        console.log(data);
        return data;
    } catch (error) {
        console.error('Scraper stats api call failure.  (Traceback next line)');
        console.log(error);
...
async function fetch_battlemetrics_servers(
    ...
    if (use_default) {
        ...
        const api_call = `https://api.battlemetrics.com/servers?${queryString.stringify(
            queryParams,
        )}`;
    ...
    next_url =
        'next' in bm_output['links'] ? bm_output['links']['next'] : 'none';
    prev_url =
        'prev' in bm_output['links'] ? bm_output['links']['prev'] : 'none';
...
async function fetch_recent_wipes_from_db(page = 1, items_per_page = 25) {
    ...
    try {
        ...
        console.error(
            'Error in fetching recent wipes from DB. (Traceback next line)',
        );
        console.log(error);
...
async function fetch_server_networks_data() {
    ...
    try {
        ...
        console.error(
            'Error in fetching server networks data. (Traceback next line)',
        );
        console.log(error);
